...
const CONFIG = {
  ...
  HOTELS_TIMEOUT: 20000, // milliseconds
 ...
 */
async function fetchHotels(lat, lon, destinationName = '') {
  const query = `
    [out:json][timeout:25];
    (
      node["tourism"~"hotel|motel|guest_house|hostel|apartment"](around:${CONFIG.SEARCH_RADIUS},${lat},${lon});
      way["tourism"~"hotel|motel|guest_house|hostel|apartment"](around:${CONFIG.SEARCH_RADIUS},${lat},${lon});
      relation["tourism"~"hotel|motel|guest_house|hostel|apartment"](around:${CONFIG.SEARCH_RADIUS},${lat},${lon});
    );
    out tags center;
  `;

  for (const endpoint of CONFIG.API.OVERPASS_ENDPOINTS) {
    try {
      const controller = new AbortController();
      const timeout = setTimeout(() => controller.abort(), CONFIG.HOTELS_TIMEOUT);
      ...
      if (!response.ok) throw new Error(`Hotels API failed (${response.status})`);

      const data = await response.json();
      if (data && Array.isArray(data.elements) && data.elements.length > 0) return data;
    } catch (error) {
      console.warn(`Hotels fetch failed on ${endpoint}:`, error);
    }
  }

  // Fallback: query Nominatim for hotel-like places around destination text.
  const fallbackData = await fetchHotelsFromNominatim(destinationName);
  if (fallbackData && Array.isArray(fallbackData.elements) && fallbackData.elements.length > 0) {
    return fallbackData;
  }

  console.error('Hotels fetch error: all providers failed');
  return { elements: [] };
}

/**
 * Fallback hotel search via Nominatim text query
 * @param {string} destinationName - Destination name
 * @returns {Promise<Object|null>} Overpass-like hotel payload
 */
async function fetchHotelsFromNominatim(destinationName) {
  const term = String(destinationName || '').trim();
  if (!term) return null;
  ...
  try {
    ...
    const response = await fetch(
      `${CONFIG.API.NOMINATIM}/search?format=json&limit=30&q=${encodeURIComponent(`hotels in ${term}`)}`,
      {
        signal: controller.signal,
    ...
    clearTimeout(timeout);
    if (!response.ok) throw new Error(`Nominatim hotels failed (${response.status})`);

    const places = await response.json();
    if (!Array.isArray(places) || places.length === 0) return { elements: [] };

    const osmTypeMap = { N: 'node', W: 'way', R: 'relation' };
    const elements = places
      .filter((p) => Number.isFinite(Number(p.lat)) && Number.isFinite(Number(p.lon)))
      .map((p) => ({
        id: p.osm_id || p.place_id,
        type: osmTypeMap[p.osm_type] || 'node',
        lat: Number(p.lat),
        lon: Number(p.lon),
        tags: {
          name: p.name || (typeof p.display_name === 'string' ? p.display_name.split(',')[0] : 'Hotel'),
    ...
    console.warn('Nominatim fallback hotels fetch error:', error);
    return null;
  }
}
...
async function fetchAndDisplayHotels(coords, searchId) {
  ...
  const data = await fetchHotels(coords.lat, coords.lon, coords.name);
  if (searchId !== ACTIVE_SEARCH_ID) return false;
  ...
  const hotels = data.elements
    .map((hotel) => {
      ...
      const bookingUrl = `https://www.booking.com/searchresults.html?ss=${encodeURIComponent(`${name} ${coords.name || ''}`.trim())}`;

      return { id: hotel.id, type: hotel.type, name, osmUrl, reviewsUrl, bookingUrl, distanceM, stars, score, tags: hotel.tags || {} };
    })
    .filter(Boolean)
      ...
      <li class="hotel-card">
        <div class="hotel-top">
          ...
          <div class="hotel-actions">
            ...
            <a class="hotel-action-link" href="${h.bookingUrl}" target="_blank" rel="noopener noreferrer" title="Search this hotel on Booking.com">Booking</a>
          </div>
        </div>
...
// Auto slide every 4 seconds - continues regardless of scroll position
slideshowInterval = setInterval(() => {
  changeSlide(1);
}, 4000);

// Ensure slideshow continues even if page is hidden (tab switching)
