...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Get guardian ID
    $guardian_id = intval($_POST['guardian_id'] ?? 0);
    ...
    try {
        // SQL query to update student guardian information
        $sql = "UPDATE student_guardians SET 
                    present_address = '$present_address',
                    present_city_id = $present_city_id,
                    present_area_id = " . ($present_area_id ?? 'NULL') . ",
                    present_country = '$present_country',
                    present_province = '$present_province',
                    permanent_address = '$permanent_address',
                    permanent_city_id = " . ($permanent_city_id ?? 'NULL') . ",
                    permanent_area_id = " . ($permanent_area_id ?? 'NULL') . ",
                    permanent_country = '$permanent_country',
                    permanent_province = '$permanent_province',
                    father_name = '$father_name',
                    father_cnic = '$father_cnic',
                    father_mobile = '$father_mobile',
                    father_mobile_operator = " . ($father_mobile_operator ?? 'NULL') . ",
                    father_sms = $father_sms,
                    father_whatsapp = $father_whatsapp,
                    father_whatsapp_number = '$father_whatsapp_number',
                    ...
                    mother_mobile_operator = " . ($mother_mobile_operator ?? 'NULL') . ",
                    mother_sms = $mother_sms,
                    mother_whatsapp = $mother_whatsapp,
                    mother_whatsapp_number = '$mother_whatsapp_number',
                    ...
                    guardian_mobile_operator = " . ($guardian_mobile_operator ?? 'NULL') . ",
        ...
        if (mysqli_query($conn, $sql)) {
            if (mysqli_affected_rows($conn) > 0) {
                ...
                $checkSql = "SELECT id FROM student_guardians WHERE id = $guardian_id";
                $checkResult = mysqli_query($conn, $checkSql);
                if (mysqli_num_rows($checkResult) > 0) {
                    // Record exists but no changes made
                    mysqli_commit($conn);
                    $response['success'] = true;
                    $response['message'] = 'No changes were made to the guardian record.';
                } else {
