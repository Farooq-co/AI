<?php
// ajax/get_guardian_details.php
header('Content-Type: application/json');
session_start();

include '../../connect.php';

$response = ['success' => false, 'message' => ''];

if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['guardian_id'])) {
    $guardianId = intval($_POST['guardian_id']);
    
    if ($guardianId <= 0) {
        $response['message'] = 'Invalid guardian ID';
        echo json_encode($response);
        exit;
    }
    
    $query = "SELECT 
                sg.id,
                sg.father_name, sg.father_cnic, sg.father_mobile, sg.father_email,
                sg.father_whatsapp_number, sg.father_profession, sg.father_education,
                sg.mother_name, sg.mother_cnic, sg.mother_mobile, sg.mother_email,
                sg.mother_whatsapp_number, sg.mother_profession, sg.mother_education,
                sg.guardian_name, sg.guardian_cnic, sg.guardian_mobile, sg.guardian_email,
                sg.guardian_whatsapp_number, sg.guardian_profession, sg.guardian_education,
                sg.present_address, sg.present_city_id, sg.present_area_id, 
                sg.present_country, sg.present_province,
                sg.permanent_address, sg.permanent_city_id, sg.permanent_area_id,
                sg.permanent_country, sg.permanent_province
              FROM student_guardians sg
              WHERE sg.id = $guardianId
              LIMIT 1";
    
    $result = mysqli_query($conn, $query);
    
    if ($result && mysqli_num_rows($result) > 0) {
        $guardian = mysqli_fetch_assoc($result);
        
        // Get city names
        if ($guardian['present_city_id']) {
            $cityQuery = "SELECT name FROM cities WHERE id = " . intval($guardian['present_city_id']);
            $cityResult = mysqli_query($conn, $cityQuery);
            if ($cityResult && mysqli_num_rows($cityResult) > 0) {
                $city = mysqli_fetch_assoc($cityResult);
                $guardian['present_city_name'] = $city['name'];
            }
        }
        
        if ($guardian['permanent_city_id']) {
            $cityQuery = "SELECT name FROM cities WHERE id = " . intval($guardian['permanent_city_id']);
            $cityResult = mysqli_query($conn, $cityQuery);
            if ($cityResult && mysqli_num_rows($cityResult) > 0) {
                $city = mysqli_fetch_assoc($cityResult);
                $guardian['permanent_city_name'] = $city['name'];
            }
        }
        
        // Get area names
        if ($guardian['present_area_id']) {
            $areaQuery = "SELECT name FROM areas WHERE id = " . intval($guardian['present_area_id']);
            $areaResult = mysqli_query($conn, $areaQuery);
            if ($areaResult && mysqli_num_rows($areaResult) > 0) {
                $area = mysqli_fetch_assoc($areaResult);
                $guardian['present_area_name'] = $area['name'];
            }
        }
        
        if ($guardian['permanent_area_id']) {
            $areaQuery = "SELECT name FROM areas WHERE id = " . intval($guardian['permanent_area_id']);
            $areaResult = mysqli_query($conn, $areaQuery);
            if ($areaResult && mysqli_num_rows($areaResult) > 0) {
                $area = mysqli_fetch_assoc($areaResult);
                $guardian['permanent_area_name'] = $area['name'];
            }
        }
        
        $response['success'] = true;
        $response['guardian'] = $guardian;
        $response['message'] = 'Guardian found successfully';
    } else {
        $response['message'] = 'Guardian not found';
    }
} else {
    $response['message'] = 'Invalid request';
}

mysqli_close($conn);
echo json_encode($response);
?>